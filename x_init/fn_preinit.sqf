...
#include "d_sm_classes_rhs.sqf"
	...
	// type of enemy chopper that will fly over the main target
	d_airai_attack_chopper = switch (d_enemy_side_short) do {
		case "E": {["O_Heli_Light_02_F","CUP_O_Mi24_P_RU","CUP_O_Mi24_V_RU","CUP_O_Ka50_SLA"]};
...
#else
	d_light_attack_chopper = switch (d_enemy_side_short) do {
		case "E": {["O_Heli_Light_02_F", "CUP_O_Mi8_RU"]};
		case "W": {["B_Heli_Light_01_F"]};
		case "G": {["I_Heli_light_03_F"]};
	};
#endif
...
if (hasInterface) then {
	...
	if (isNil "d_uid_reserved_slots") then {
		d_uid_reserved_slots = ["d_artop_01"];
		d_uids_for_reserved_slots = ["765611980988345261"];
	};
	...
	// is engineer
#ifndef __TT__
	d_is_engineer = ["d_artop_01","d_delta_1","d_delta_2","d_delta_3","d_delta_4","d_delta_5","d_delta_6"];
#else
	d_is_engineer = ["d_artop_01","d_blufor_17","d_blufor_18","d_blufor_19","d_opfor_17","d_opfor_18","d_opfor_19"];
#endif

	// can call in air drop
#ifndef __TT__
	d_can_call_drop_ar = ["d_artop_01","d_alpha_1","d_charlie_1","d_echo_1"];
#else
	d_can_call_drop_ar = [];
#endif
	...
	d_only_pilots_can_fly = ["pilot_1","pilot_2","pilot_3","pilot_4","pilot_5"];
